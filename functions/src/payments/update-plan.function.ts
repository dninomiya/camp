...
export const updatePlan = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new Error('ユーザーが存在しません');
  }

  const uid = context.auth.uid;
  const connectData: any = (await db.doc(`users/${uid}/private/connect`).get()).data();

  if (!connectData || !connectData.stripeUserId) {
    throw new Error('ストライプアカウントが存在しません');
  }

  return stripe.plans.update(
    data.id,
    {
