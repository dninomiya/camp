...
export const deletePlan = functions.https.onCall(async (data, context) => {
  ...
  const paymentData: any = (await db.doc(`users/${context.auth.uid}/private/payment`).get()).data();

  if (!paymentData || !paymentData.stripeUserId) {
    throw new Error('ストライプアカウントが存在しません');
  }

  await stripe.plans.delete(
    data.id,
    {
  ...
  const members = await db.collection(`channels/${data.channelId}/plans/${data.pid}/members`).get()
  ...
  return firebaseTools.firestore.delete(`channels/${data.channelId}/plans/${data.pid}`, {
