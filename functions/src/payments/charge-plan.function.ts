...
export const chargePlan = functions.region('asia-northeast1').https.onCall(
  async (
    data: {
      item: {
        id: string;
    ...
    const { item } = data;
    const connect = (
      await db.doc(`users/${data.sellerUid}/private/connect`).get()
    ).data();
    const sellerId = connect && connect.stripeUserId;
    const customer = (
      await db.doc(`users/${data.customerUid}/private/payment`).get()
    ).data();
