...
export const addLike = functions
  ...
  .firestore.document('channels/{cid}/likes/{lessonId}')
  .onCreate(async (snap, context) => {
    await countUp(`lessons/${context.params.lessonId}`, 'likeCount');
    const lesson = (
      await db.doc(`lessons/${context.params.lessonId}`).get()
    ).data();
    if (lesson) {
      await countUp(
        `channels/${lesson.channelId}`,
        'statistics.totalLikedCount'
      );
    }
    return countUp(
      `channels/${context.params.cid}`,
...
export const follow = functions
  .region('asia-northeast1')
  .firestore.document('channels/{cid}/followers/{uid}')
  .onCreate(async (snap, context) => {
    await db
      .doc(`channels/${context.params.uid}/follows/${context.params.cid}`)
      .set({
...
export const unfollow = functions
  .region('asia-northeast1')
  .firestore.document('channels/{cid}/followers/{uid}')
  .onDelete(async (snap, context) => {
    await db
